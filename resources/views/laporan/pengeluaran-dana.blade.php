    ...
    <table style="width:100%;border-collapse:collapse;">
        <tr>
            ...
            <td style="text-align:center;">
                ...
                <div class="report-meta">SMA UNGGULAN BPPT DARUS SHOLAH | Laporan Pengeluaran Dana</div>
...
@php
    $dokumenPengeluaran = 'LPG-' . now()->format('Ymd') . '-' . str_pad((string) $data->count(), 3, '0', STR_PAD_LEFT);
    $nomorSuratPengeluaran = 'DS/KEU-K/' . now()->format('Y/m') . '/' . str_pad((string) $data->count(), 3, '0', STR_PAD_LEFT);
@endphp
...
<div class="print-only mb-3" style="font-size:12px;color:#2f4564;line-height:1.7;">
    ...
    <div style="text-align:justify;margin-bottom:8px;">Bersama ini kami sampaikan laporan pengeluaran dana pada periode {{ $periode['label'] }} untuk kebutuhan monitoring realisasi penggunaan anggaran yayasan.</div>
    <div><strong style="display:inline-block;min-width:180px;">Jenis Dokumen</strong>: Laporan Pengeluaran Dana</div>
    <div><strong style="display:inline-block;min-width:180px;">Nomor Dokumen</strong>: {{ $dokumenPengeluaran }}</div>
    <div><strong style="display:inline-block;min-width:180px;">Total Pengeluaran</strong>: Rp {{ number_format($totalPengeluaran, 0, ',', '.') }}</div>
</div>
